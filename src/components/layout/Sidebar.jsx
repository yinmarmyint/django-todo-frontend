...
import businessLogo from '../../static/images/comb1.png';
...
const NavItem = ({
  ...
  if (sidebar && _.intersection(authUser.authorities, hasAnyRoles).length > 0) {
    return (
      <Whisper
        placement="right"
        trigger="hover"
...
const Sidebar = props => {
  const { expand, authUser, routes: unSortedRoutes } = props;
  ...
  return (
    ...
    >
      ...
      >
        <Sidenav.Header style={{ width: '100%' }}>
          {expand ? (
            <BusinessInfo authUser={authUser} expand={props.expand} />
          ) : (
            <>
              <img src={businessLogo} alt="not available" width="50" />
            </>
          )}
        </Sidenav.Header>
        <Sidenav.Body>
          {/* <div
            className={expand ? styles.scrollbar : styles.noscroll}
            id={styles.scroll}
          > */}
          <Nav
            className={
